...
#define kCameraToolBarHeight 60
...
//    [self adjustPossition];
    ...
    CGRect cropFrame=CGRectMake(_sourceImageView.contentFrame.origin.x + 20 ,_sourceImageView.contentFrame.origin.y+64,_sourceImageView.contentFrame.size.width,_sourceImageView.contentFrame.size.height);
...
-(void)initCropFrame{
    _sourceImageView = [[UIImageView alloc] initWithFrame:CGRectMake(20, 64, self.view.bounds.size.width - 40, self.view.bounds.size.height-kCameraToolBarHeight-64)];
    [_sourceImageView setContentMode:UIViewContentModeScaleAspectFit];
    _sourceImageView.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:1];
...
-(void)singlePan:(UIPanGestureRecognizer *)gesture{
    ...
    if(gesture.state==UIGestureRecognizerStateEnded){
        ...
        _cropRect.activePoint.backgroundColor = [UIColor clearColor];
...
//        CGRectMake(_sourceImageView.contentFrame.origin.x + 20,_sourceImageView.contentFrame.origin.y+64,_sourceImageView.contentFrame.size.width,_sourceImageView.contentFrame.size.height);
        
        [_cropRect topLeftCornerToCGPoint:CGPointMake(0, 0)];
        [_cropRect topRightCornerToCGPoint:CGPointMake(_sourceImageView.contentFrame.size.width, 0)];
        [_cropRect bottomRightCornerToCGPoint:CGPointMake(_sourceImageView.contentFrame.size.width, _sourceImageView.contentFrame.size.height)];
        [_cropRect bottomLeftCornerToCGPoint:CGPointMake(0, _sourceImageView.contentFrame.size.height)];
...
//        [[NSNotificationCenter defaultCenter] postNotificationName:LXCamScan object:_cropImage];
        
        [self.cropdelegate didFinishCropping:_cropImage from:self];
...
//         _sourceImageView.image = [MMOpenCVHelper UIImageFromCVMat:grayImage];//For gray image
    ...
    undistorted.release();
        ...
        UIAlertView  *alertView = [[UIAlertView alloc] initWithTitle:@"档案APP相机" message:@"无效的尺寸" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
        [alertView show];
...
- (IBAction)leftAC:(UIButton *)sender {
